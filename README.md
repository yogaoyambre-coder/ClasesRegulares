...
- "Editar" (en la ficha del cliente o desde una fila de Cobros) es el único sitio donde se gestiona todo sobre un cliente: sus datos básicos, asignar centro+horario (añadir clase), dar de baja un grupo concreto, marcar/desmarcar tarifa especial por centro, y dar de baja, reactivar o eliminar al cliente entero cuando no le queda ninguna clase activa en ningún centro.
- Por defecto un cliente nunca se borra: pasa a estado "baja" (o su última inscripción activa se da de baja) y su ficha se conserva por si vuelve. "Eliminar definitivamente" es la excepción: borra el cliente y, en cascada, todas sus inscripciones, pagos y registros de asistencia — irreversible, pensado para fichas erróneas o duplicadas, no para clientes reales que dejaron de venir (para esos, dar de baja).
